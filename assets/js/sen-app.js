    (() => {
      ...
      function clearSharedDeviceRecord() { if (!confirm('確定清除這部裝置的課堂紀錄嗎？清除後不能復原。')) return; try { localStorage.removeItem(lessonStorageKey); } catch (_) {} lessonSession = createLessonSession(); renderLessonSession(); showToast('已清除本機課堂紀錄；可以安全交給下一位使用者。'); }
      ...
      function updateGameLibraryHeading() {
        if (activePathway) {
          ...
          if (activePathway === 'G' && (window.GIFTED_2E_LAB || window.GIFTED_CROSS_SEN_LAB || window.GIFTED_EIGHT_GAMES_LAB || window.GIFTED_ATTACHMENT_ARCADE)) {
            ...
            const attachmentCount = window.GIFTED_ATTACHMENT_ARCADE?.activityCards(activeStage).length || 0;
            const count = coreCount + crossCount + giftedCount + attachmentCount;
            const focus = activeStage === 'lower' ? '彈性建構、感官自主、創意改造與視覺規律' : activeStage === 'upper' ? '多角度推理、系統設計、科學探究與媒體素養' : activeStage === 'junior' ? '研究方法、資料倫理、論證結構與多元觀點' : '未來情境、學術評審、AI 倫理與包容領導';
      ...
      function renderGameLibrary(filter = activeFilter) {
        ...
        const isGifted2eDirectSelect = activePathway === 'G' && Boolean(window.GIFTED_2E_LAB || window.GIFTED_CROSS_SEN_LAB || window.GIFTED_EIGHT_GAMES_LAB || window.GIFTED_ATTACHMENT_ARCADE);
        ...
        const p1SafetyCards = !activePathway && window.P1_SAFETY_TRANSITION_LAB && (filter === 'all' || filter === 'emotion' || filter === 'cognition' || filter === 'life') ? window.P1_SAFETY_TRANSITION_LAB.activityCards(activeStage).filter((card) => filter === 'all' || (filter === 'cognition' && card.p1Topic === 'digital') || (filter === 'life' && card.p1Topic === 'change') || (filter === 'emotion' && ['advocate', 'repair'].includes(card.p1Topic))).map((card) => `<button class="game-card p1-safety-direct-card" type="button" data-p1-topic="${card.p1Topic}" data-tone="${card.tone}"><div class="game-visual" aria-hidden="true">${card.icon}</div><h3>${card.title}</h3><p>${card.description}</p><div class="support-badge-row"><span class="support-badge">跨類別 P1 題材包</span><span class="support-badge">可選朗讀</span><span class="support-badge">不收集資料</span></div><span class="tag">${stageProfiles[activeStage].label} · 8 個虛構情境</span></button>`).join('') : '';
        const practicalLiteracyCards = !activePathway && window.PRACTICAL_LITERACY_LAB && (filter === 'all' || filter === 'life') ? window.PRACTICAL_LITERACY_LAB.activityCards(activeStage).map((card) => `<button class="game-card practical-literacy-direct-card" type="button" data-practical-pack="${card.practicalPack}" data-tone="${card.tone}"><div class="game-visual" aria-hidden="true">${card.icon}</div><h3>${card.title}</h3><p>${card.description}</p><div class="support-badge-row"><span class="support-badge">跨類別 ${card.phase || 'P2'} 題材包</span>${(card.badges || []).map((badge) => `<span class="support-badge">${badge}</span>`).join('')}</div><span class="tag">${stageProfiles[activeStage].label} · 8 個虛構情境</span></button>`).join('') : '';
        $('#gameGrid').innerHTML = crossCategoryDirectCard + p1SafetyCards + practicalLiteracyCards + asdFifteenDirectCard + asdDirectCard + asdEightDirectCard + adhdFifteenDirectCard + adhdDirectCard + idFifteenDirectCard + idDirectCard + idEightDirectCard + idAdvancedDirectCard + sliFifteenDirectCard + miFifteenDirectCard + hiEightDirectCard + viDirectCard + pdDirectCard + games.map(game => {
        ...
        $$('.game-card').forEach(card => card.addEventListener('click', () => {
          ...
          if (card.dataset.p1Topic) {
            window.P1_SAFETY_TRANSITION_LAB?.openTopic(card.dataset.p1Topic, { stage: activeStage, trigger: card });
            return;
          }
          if (card.dataset.practicalPack) {
            window.PRACTICAL_LITERACY_LAB?.open(card.dataset.practicalPack, { stage: activeStage, trigger: card });
            return;
          }
          ...
          if (card.dataset.giftedAttachmentActivity) {
            if (!window.GIFTED_ATTACHMENT_ARCADE) { showToast('資優／雙殊任務工房正在準備中，請稍後再試。'); return; }
            window.GIFTED_ATTACHMENT_ARCADE.openActivity(card.dataset.giftedAttachmentActivity, { stage: activeStage, onComplete: recordGifted2eLabResult, trigger: card });
            return;
          }
