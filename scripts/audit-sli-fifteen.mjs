...
const source = fs.readFileSync(new URL('../modules/sli/sli-fifteen-catalogue-lab.js', import.meta.url), 'utf8');
const context = { window: {}, document: {}, console, setTimeout, clearTimeout };
context.window.window = context.window;
vm.createContext(context);
vm.runInContext(source, context, { filename: 'sli-fifteen-catalogue-lab.js' });

const lab = context.window.SLI_FIFTEEN_CATALOGUE_LAB;
if (!lab) throw new Error('找不到 SLI_FIFTEEN_CATALOGUE_LAB API');
...
if (report.some((row) => row.count !== 15 || row.minRounds < 8 || row.underEight.length)) {
  throw new Error('SLI 十五項活動稽核失敗');
}
console.log('SLI fifteen catalogue audit: OK');
