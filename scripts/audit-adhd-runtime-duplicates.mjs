...
const port = 9479;
const reportPath = '/home/ubuntu/sen_application/ADHD_RUNTIME_DUPLICITY_AUDIT_20260824.json';
const url = 'file:///home/ubuntu/sen_application/index.html?adhdRuntimeDuplicateAudit=20260824';
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const chrome = spawn('chromium', ['--headless', '--no-sandbox', '--disable-gpu', '--allow-file-access-from-files', `--remote-debugging-port=${port}`, '--user-data-dir=/tmp/adhd-runtime-duplicate-audit', 'about:blank'], { stdio: 'ignore' });
...
async function targetUrl() {
  ...
  throw new Error('無法連接 ADHD 題庫掃描瀏覽器。');
}
...
function connect(webSocketUrl) {
  return new Promise((resolve, reject) => {
    ...
    socket.addEventListener('error', () => reject(new Error('ADHD 題庫掃描瀏覽器連線失敗。')));
  });
}
...
try {
  ...
  const activities = await evaluate(client, `(() => {
    ...
    const taskFingerprint = (round) => JSON.stringify(normalise({
      scene: round.scene || '', prompt: round.prompt || '', context: round.context || '',
      instruction: round.instruction || '', answer: round.answer || '', target: round.target || ''
    }));
    const collect = (stage, source, activities) => activities.map((activity) => {
      ...
      return { stage, source, id: activity.id, title: activity.title, rounds: rounds.length, uniqueRounds: new Set(prints).size, uniqueTasks: new Set(taskPrints).size, duplicateIndexes };
    });
    const stageKeys = [['lower', '初小 P1–P3'], ['upper', '高小 P4–P6'], ['junior', '初中 S1–S3'], ['senior', '高中 S4–S6']];
    const stageTasks = stageKeys.flatMap(([key, label]) => collect(label, '主路線', [{ id: 'adhd-stage-' + key, title: window.ADHD_STAGE_TASKS?.[key]?.title || 'ADHD 主路線', rounds: window.ADHD_STAGE_TASKS?.[key]?.rounds || [] }]));
    const catalogue = stageKeys.flatMap(([key, label]) => collect(label, '十五項直接選關', window.ADHD_FIFTEEN_CATALOGUE_LAB?.activityCards?.(key) || []));
    return [...stageTasks, ...catalogue];
  })()`);
