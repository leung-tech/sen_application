...
const packs = { finance: { label: '日常金錢與消費決策', expectedContext: '虛構消費情境' }, body: { label: '身體訊號、休息與日常健康溝通', expectedContext: '虛構日常健康溝通情境' } };
...
async function auditEscape(client, key) {
  await navigate(client);
...
try {
  ...
  const mobile = []; for (const key of Object.keys(packs)) for (const stage of stages) mobile.push(await auditMobile(client, key, stage)); await client.call('Emulation.clearDeviceMetricsOverride');
  await navigate(client);
  const structural = []; for (const key of Object.keys(packs)) for (const stage of stages) structural.push(await evaluate(client, `(() => { const source = window.PRACTICAL_LITERACY_LAB?.packInfo?.(${JSON.stringify(key)}) || {}; const rounds = window.PRACTICAL_LITERACY_LAB?.roundsFor(${JSON.stringify(key)}, ${JSON.stringify(stage)}) || []; const positions = rounds.map((round) => round.choices.indexOf(round.answer)); const text = rounds.map((round) => [round.title, round.scene, round.prompt, round.hint].join('｜')).join('\\n'); const safety = [source.intro, ...(source.safetyBullets || []), source.finishMessage].join('｜'); return { key: ${JSON.stringify(key)}, stage: ${JSON.stringify(stage)}, count: rounds.length, ids: rounds.map((round) => round.id), distinct: new Set(rounds.map((round) => round.title + '｜' + round.scene)).size, choicesValid: rounds.every((round) => round.choices.length === 3 && new Set(round.choices).size === 3 && round.choices.includes(round.answer)), positions, balanced: positions.every((position, index) => position === index % 3), financeCollectionRequest: /((請|必須|先).{0,12}(輸入|填寫|上傳|提供)).{0,30}(銀行|信用卡|八達通|帳戶|密碼|電話|地址|付款資料)/.test(text), prohibitedFinanceAdvice: /(投資建議|信貸建議|借貸建議|賭博建議)/.test(text), bodySafety: { noData: /不會要求.{0,24}(真實症狀|病歷|用藥|健康資料)/.test(safety), noDiagnosis: /不作.{0,12}診斷/.test(safety), noTreatment: /不提供.{0,16}(服藥|用藥).{0,12}(治療)?指示/.test(safety), urgentSupport: /(急切安全疑慮|需要立即協助).{0,42}(可信任成人|緊急服務)/.test(safety) } }; })()`));
  const flows = []; for (const key of Object.keys(packs)) for (const stage of stages) flows.push(await completeStage(client, key, stage));
  const escape = []; for (const key of Object.keys(packs)) escape.push(await auditEscape(client, key));
  const failures = [];
  structural.forEach((entry) => { if (entry.count !== 8 || new Set(entry.ids).size !== 8 || entry.distinct !== 8) failures.push(`${entry.key}／${entry.stage}：未有八個不同情境。`); if (!entry.choicesValid || !entry.balanced) failures.push(`${entry.key}／${entry.stage}：三欄選項或左中右答案位置不正確。`); if (entry.key === 'finance' && (entry.financeCollectionRequest || entry.prohibitedFinanceAdvice)) failures.push(`${entry.key}／${entry.stage}：出現不符合金錢資料或建議安全界線的文字。`); if (entry.key === 'body' && (!entry.bodySafety.noData || !entry.bodySafety.noDiagnosis || !entry.bodySafety.noTreatment || !entry.bodySafety.urgentSupport)) failures.push(`${entry.key}／${entry.stage}：缺少健康資料、診斷／治療或急切安全求助界線。`); });
