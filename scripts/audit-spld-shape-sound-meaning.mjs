...
const report = ['lower', 'upper', 'junior', 'senior'].map((stage) => {
  ...
  return { stage, count: rounds.length, missingFields: rounds.filter((round) => round.length !== 5 || round.some((value) => !value)).length };
});
for (const row of report) if (row.count !== 8 || row.missingFields) throw new Error(`${row.stage} 的形音義資料必須有 8 個完整漢字。`);
if (!source.includes('id="spldSmDock"') || !source.includes('draggable="true"') || !source.includes('線索 ${phase + 1} / 3')) throw new Error('形音義工房缺少可選拖放、線索放大鏡或三步進度。');
