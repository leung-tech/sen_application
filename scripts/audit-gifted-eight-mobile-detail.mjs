...
try {
  ...
  const failures = [];
  menus.forEach((menu) => { const expected=[...activities[menu.stage], 'open-inquiry']; if (menu.documentOverflow || menu.cards.length!==expected.length || expected.some((id)=>!menu.cards.some((card)=>card.id===id)) || menu.cards.some((card)=>card.right>375||card.h<40||card.overflow)) failures.push(`${menu.stage} 直接選關：溢出、觸控範圍或卡片尺寸不符合。`); });
