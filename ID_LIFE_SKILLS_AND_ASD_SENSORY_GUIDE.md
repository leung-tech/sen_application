...
| 超市收銀小站 | 不在初小／高小顯示 | 初中／高中：掃描商品、收 20 元、核對找續 | 讓學生每次只做一項工作；找續題全以可見數字呈現。 |
| 圖書館分類工作 | 不在初小／高小顯示 | 初中／高中：看主題、選書架、簡短交接 | 書架標籤和圖示同時出現；交接可讀或指向整句。 |
...
「超市收銀小站」使用一張商品工作單。學生先由四項商品中找出客人放到櫃位的物品，再按「收 20 元」，最後在可見的金額選項中選擇找續。教師可只帶學生完成掃描，也可在學生準備好時延伸至收款和找續；每個步驟均可重讀、暫停或重試。

「圖書館分類工作」讓學生由書名和圖示判斷主題，將書放進科學、社區、自然或生活書架。放好後，學生以一句可見的短句「《書名》已放好，請你核對」完成交接。若學生不想說話，可直接按整句或指向螢幕，教師再代為朗讀。
...
公開 GitHub Pages 已以 Chromium 與 Chrome DevTools Protocol 驗證四個學段的 ID 遊戲流程：初小與高小各五項，初中與高中各八項，合共 **26 個流程**。稽核覆蓋直接入口、三步準備頁、動態狀態訊息、進度列、Tab／Shift+Tab、Escape、焦點回復、375px 手機版大型控制、超市收銀商品掃描、圖書館書架分類及 ASD 高年級感官自主設定，最終結果為 **`failureCount: 0`**。可重跑腳本位於 [`scripts/audit-id-sensory-labs.mjs`](scripts/audit-id-sensory-labs.mjs)。
