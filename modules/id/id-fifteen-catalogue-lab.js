...
(function () {
  ...
  const fourColumnPatterns = [
    [3,1,0,2,3,0,1,2], [2,0,3,1,2,1,0,3], [1,3,0,2,1,0,3,2], [0,2,1,3,0,3,2,1]
  ];
  ...
  applyPositionPatterns(senior, 3);
  ...
  const orderedChoices = (round) => {
    const choices = [...round.choices]; const answer = round.answer; const pattern = active?.answerPositionPattern;
    const position = pattern?.[roundIndex % pattern.length];
    if (!Number.isInteger(position) || position < 0 || position >= choices.length || !choices.includes(answer)) return choices;
    const otherChoices = choices.filter((choice) => choice !== answer); const shown = [];
    for (let index = 0; index < choices.length; index += 1) shown[index] = index === position ? answer : otherChoices.shift();
    return shown;
  };
