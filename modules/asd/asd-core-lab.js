(() => {
  const GRADE = {
    lower: { label: '初小 · P1–P3', emotionCount: 3, rounds: 4, jointTargets: 3, visualMode: 'detail', sensoryCount: 3 },
    upper: { label: '高小 · P4–P6', emotionCount: 5, rounds: 5, jointTargets: 4, visualMode: 'switch', sensoryCount: 4 },
    junior: { label: '初中 · S1–S3', emotionCount: 5, rounds: 6, jointTargets: 4, visualMode: 'switch', sensoryCount: 5 },
    senior: { label: '高中 · S4–S6', emotionCount: 6, rounds: 6, jointTargets: 4, visualMode: 'switch', sensoryCount: 5 },
  };
  ...
  function renderStory() {
    const item = STORIES[state.index % STORIES.length]; state.answer = item.answer;
    shell(`${top(GAMES.story.title, '這不是猜別人唯一的想法；我們一起從情境找一個可能感受和尊重的下一步。', `${currentGrade().label} · ${state.index + 1} / ${state.total}`)}${progress()}<article class="story-card"><span>校園小故事</span><p>${item.context}</p><strong>${item.question}</strong></article><div class="asd-rule">先想一想這件事可能帶來甚麼感受或影響，再選擇最有幫助的回應。</div><div class="asd-choice-grid">${shuffle(item.options).map((choice) => `<button type="button" class="asd-choice" data-story="${choice}">${choice}</button>`).join('')}</div><div id="asdLabFeedback" class="asd-feedback" role="status" aria-live="polite" aria-atomic="true">先讀情境；不確定時可以請教師一起讀一遍。</div>`);
    qa('[data-story]').forEach((button) => button.addEventListener('click', () => { if (button.dataset.story === item.answer) { state.correct += 1; state.tries += 1; feedback(`✓ ${item.why} 可以嘗試說：「${item.next}」`, 'ok', 'story'); wait(() => next(renderStory), 1050); } else { state.tries += 1; feedback(`↗ 先想一想：${item.why}`, 'try'); } })); focusSoon('[data-story]');
  }

  function renderJoint() {
    const directions = ['left', 'right', 'up', 'down']; const direction = directions[(state.index + Math.floor(Math.random() * 4)) % 4]; const labels = { left: '左邊', right: '右邊', up: '上面', down: '下面' }; state.answer = direction;
    const boxes = shuffle(directions.slice(0, currentGrade().jointTargets).map((item) => item === direction ? item : item)); if (!boxes.includes(direction)) boxes[0] = direction;
