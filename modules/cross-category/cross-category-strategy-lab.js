(function () {
  ...
  const TRACKS = {
    emotion: {
      icon: '🤝',
      title: '同理與協商工房',
      subtitle: '八個虛構角色情境任務：看不同角度，把支持、修復或求助策略卡放到下一步收集盒。',
      cards: [
  ...
  function bindMissionBoard() {
    const dock = q('#crossDock');
    if (!dock) return;
    dock.addEventListener('dragover', (event) => { event.preventDefault(); dock.classList.add('is-over'); });
    ...
    qa('[data-cross-choice]').forEach((button) => {
      button.addEventListener('dragstart', (event) => {
        event.dataTransfer?.setData('text/plain', button.dataset.crossChoice || '');
