(function () {
  ...
  const speak = (text) => {
    ...
    utterance.lang = 'zh-HK'; utterance.rate = 0.72; window.speechSynthesis.speak(utterance);
  };
  const closeLab = () => { window.speechSynthesis?.cancel(); document.querySelector('.spld-s4-lab-backdrop')?.remove(); };
  ...
  function choiceMarkup(round) { return `<div class="spld-s4-choice-grid ${currentActivity().layout === 'long' ? 'long' : ''}">${round.choices.map((choice, index) => `<button type="button" class="spld-s4-choice" data-s4-choice="${choice}"><span>${index + 1}</span><strong>${choice}</strong></button>`).join('')}</div>`; }
  function renderRound() {
    ...
    lab.innerHTML = `<button class="spld-s4-close" type="button" aria-label="關閉高中讀寫實驗室">×</button><div class="spld-s4-heading compact"><span>${activity.focus}</span><h2>${activity.icon} ${activity.title}</h2><p>${activity.description}</p></div><div class="spld-s4-progress"><b>第 ${roundIndex + 1} / ${total} 關</b><i><em style="width:${((roundIndex + 1) / total) * 100}%"></em></i></div><article class="spld-s4-scene"><small>${round.lead}</small><p>${round.context}</p></article><p class="spld-s4-prompt">${round.prompt}</p>${choiceMarkup(round)}<div class="spld-s4-feedback" id="spldS4Feedback">慢慢看一看；不知道時可以按提示。</div><div class="spld-s4-tools"><button type="button" id="spldS4Read">🔊 朗讀題目</button><button type="button" id="spldS4Hint">💡 看提示</button><button type="button" id="spldS4Back">← 換一項練習</button></div>`;
    lab.querySelector('.spld-s4-close')?.addEventListener('click', closeLab);
    lab.querySelector('#spldS4Read')?.addEventListener('click', () => speak(`${activity.title}。${round.lead}。${round.context}。${round.prompt}`));
