(function () {
  ...
  const closeLab = () => { stopReading(); document.querySelector('.spld-s4-lab-backdrop')?.remove(); };
  ...
  function renderRound() {
    ...
    lab.innerHTML = `<button class="spld-s4-close" type="button" aria-label="關閉高中讀寫實驗室">×</button><div class="spld-s4-heading compact"><span>${activity.focus}</span><h2>${activity.icon} ${activity.title}</h2><p>${activity.description}</p></div><div class="spld-s4-progress"><b>第 ${roundIndex + 1} / ${total} 關</b><i><em style="width:${((roundIndex + 1) / total) * 100}%"></em></i></div><article class="spld-s4-scene"><small>${round.lead}</small><p>${round.context}</p></article><p class="spld-s4-prompt">${round.prompt}</p><div class="spld-s4-feedback" id="spldS4Feedback" role="status" aria-live="polite">慢慢看一看；不知道時可以按提示。</div><aside class="spld-s4-low-pressure-inline">需要時可先聽語境和題目，再看提示；不需要一次完成。</aside><div class="spld-s4-tools"><button type="button" id="spldS4Read" aria-label="朗讀語境及題目" aria-pressed="false">🔊 朗讀語境及題目</button><button type="button" id="spldS4Hint" aria-label="顯示解題提示">💡 看提示</button><button type="button" id="spldS4Back">← 換一項練習</button></div>${choiceMarkup(round)}`;
