(function () {
  ...
  function choiceGridMarkup(choices) {
    return `<div class="spld-s1-choice-grid">${choices.map((choice, index) => `<button type="button" class="spld-s1-choice" data-choice="${choice}" aria-label="選項 ${index + 1}：${choice}"><span aria-hidden="true">${index + 1}</span><strong>${choice}</strong></button>`).join('')}</div>`;
  }
